...
if size(ECOC,2)>length(Classifiers) %All-Pairs approach
    avgs=ECOC(:,size(ECOC,1)+1:end);
    ECOC=ECOC(:,1:size(ECOC,1));
    for z=1:size(avgs,1)
        X(:,z)=feval(base_test,TestData(:,1:end-1)-repmat(avgs(z,:),size(TestData,1),1),Classifiers{1}.classifier,base_test_params);
    end
else %ECOC approach
    for z=1:size(ECOC,2)
         try,
...
for i=1:size(TestData,1) % for each sample in the test set
    ...
    if classes(class)==TestData(i,end)
        hits=hits+1;
    end
    confusion(find(classes==TestData(i,end)),class)=confusion(find(classes==TestData(i,end)),class)+1;
end
...
y(1:length(x))=exp((-0.5*(x.*x))/(sigma^2)); 
